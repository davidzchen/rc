...
// buffer.
static int fdgchar() {
  // Has the buffer been exhausted? if so, replenish it.
  if (chars_out >= chars_in + 2) {
    while (1) {
#if EDITLINE || READLINE
...
// Set up the input stack, and put a "dead" input at the bottom, so that 
// yyparse will always read eof.
...
// Push an input source onto the stack. set up a new input buffer, and set 
// gchar().
...
// Remove an input source from the stack. restore the right kind of getchar 
// (string,fd) etc.
...
extern Node *rc_loop(bool clobberexecit) {
  ...
  for (eof = FALSE; !eof;) {
    ...
    // "lastchar" can be clobbered during a walk().
    eof = (lastchar == EOF); 
    if (parsetree != NULL) {
      if (execit) {
        walk(parsetree, TRUE);
