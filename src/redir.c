// redir.c: code for opening files and piping heredocs after fork but before 
// exec.
  ...
  for (Rq *r = redirq; r != NULL; r = r->n) {
    ...
    case nRedir:
      if (r->r->u[0].i == rHerestring) {
        ...
        List *fname = flatten(glom(r->r->u[2].p)); 
        int p[2];
        if (pipe(p) < 0) {
          uerror("pipe");
          rc_error(NULL);
        ...
        List *fname = glob(glom(r->r->u[2].p));
        if (fname == NULL) {
